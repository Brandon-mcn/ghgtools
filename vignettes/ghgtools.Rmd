...
Let's look at the raw emission factor library, which is made available once we load ghgtools with `library(ghgtools)`
<br>
...
AR5_EFL <- create_efl("AR5")
```

Let's take a look at the new emission factor library
<br>
...
<br>
`create_efl()` let's us produce an emission factor library for reference. To calculate a GHG inventory, we use the function `ghg_inventory()`. Like `create_efl()`, this function aslo requires the selection of GWPs. 

```{r, include=TRUE, message=FALSE, warning=FALSE}
AR5_ghginventory <- ghg_inventory("AR5")
```
...
library(ggplot2)
ghg_summary <- AR5_ghginventory[, .(EmissionTotal = sum(mt_co2e)), by = emission_scope]
...
```{r, echo=FALSE, message = FALSE, warning=FALSE, fig.width=6, fig.height=4}
Ecat_summary <- AR5_ghginventory[, .(EmissionTotal = sum(mt_co2e)), by = service_type]
...
state_data <- AR5_ghginventory[emission_scope %in% scope1and2, .(EmissionTotal = sum(mt_co2e)), by = state]
